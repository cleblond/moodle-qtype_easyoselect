...
/**
 * Unit tests for the easyomech question definition class.
 *
 * @package    qtype
...
require_once($CFG->dirroot . '/question/type/easyomech/simpletest/helper.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_easyomech_question_test extends UnitTestCase {
    public function test_compare_string_with_wildcard() {
        // Test case sensitive literal matches.
        $this->assertTrue(qtype_easyomech_question::compare_string_with_wildcard(
                'Frog', 'Frog', false));
        $this->assertFalse(qtype_easyomech_question::compare_string_with_wildcard(
                'Frog', 'frog', false));
        $this->assertTrue(qtype_easyomech_question::compare_string_with_wildcard(
                '   Frog   ', 'Frog', false));
        $this->assertFalse(qtype_easyomech_question::compare_string_with_wildcard(
    ...
    public function test_is_complete_response() {
		$question = qtype_easyomech_test_helper::make_a_easyomech_question();
    ...
    public function test_get_question_summary() {
        $easyomech = qtype_easyomech_test_helper::make_a_easyomech_question();
        $qsummary = $easyomech->get_question_summary();
        $this->assertEqual('Name an amphibian: __________', $qsummary);
    }

    public function test_summarise_response() {
        $easyomech = qtype_easyomech_test_helper::make_a_easyomech_question();
        $summary = $easyomech->summarise_response(array('answer' => 'dog'));
        $this->assertEqual('dog', $summary);
    }

    public function test_classify_response() {
        $easyomech = qtype_easyomech_test_helper::make_a_easyomech_question();
        $easyomech->start_attempt(new question_attempt_step(), 1);

        $this->assertEqual(array(
                new question_classified_response(13, 'frog', 1.0)),
                $easyomech->classify_response(array('answer' => 'frog')));
        $this->assertEqual(array(
                new question_classified_response(14, 'toad', 0.8)),
                $easyomech->classify_response(array('answer' => 'toad')));
        $this->assertEqual(array(
                new question_classified_response(15, 'cat', 0.0)),
                $easyomech->classify_response(array('answer' => 'cat')));
        $this->assertEqual(array(
                question_classified_response::no_response()),
                $easyomech->classify_response(array('answer' => '')));
    }
}
